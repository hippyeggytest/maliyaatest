import  { ReceiptData } from './pdfGenerator';
import { normalizeArabicText, formatArabicDate, formatArabicCurrency } from './arabicSupport';
...
export const generatePrintReceipt = (data: ReceiptData): void => {
  ...
  const formattedDate = formatArabicDate(data.date);
      ...
      <div class="receipt-container">
        <div class="receipt-header">
          ${data.schoolLogo ? `
            <div class="logo-container">
              <img src="${data.schoolLogo}" alt="${normalizeArabicText(data.schoolName)}" />
            </div>
          ` : ''}
          <h1 class="receipt-title">إيصال استلام مدفوعات</h1>
          <p class="receipt-school">${normalizeArabicText(data.schoolName)}</p>
        </div>
        ...
        <div class="receipt-section">
          ...
          <div class="detail-row">
            ...
            <span>${normalizeArabicText(data.studentName)}</span>
          </div>
          <div class="detail-row">
            <span class="detail-label">الصف:</span>
            <span>${normalizeArabicText(data.grade)}</span>
          </div>
        </div>
        ...
        <div class="receipt-section">
          ...
          <div class="detail-row">
            ...
            <span>${normalizeArabicText(data.feeName)}</span>
          </div>
          <div class="detail-row">
            <span class="detail-label">طريقة الدفع:</span>
            <span>${normalizeArabicText(data.paymentMethod)}</span>
          </div>
          ${data.notes ? `
            <div class="receipt-notes">
              <p>ملاحظات: ${normalizeArabicText(data.notes)}</p>
            </div>
          ` : ''}
          <div class="amount-row">
            <span class="detail-label">المبلغ المدفوع:</span>
            <span class="amount-value">${formatArabicCurrency(data.amount)}</span>
          </div>
        </div>
        ...
        <div class="receipt-footer">
          ...
          <p>تاريخ الطباعة: ${formatArabicDate(new Date())}</p>
        </div>
      </div>
...
export const generateFinancialReportPrint = (data: any): void => {
  ...
  const fromDate = formatArabicDate(data.fromDate);
  const toDate = formatArabicDate(data.toDate);
  ...
  printWindow.document.write(`
    ...
    <head>
      <meta charset="UTF-8">
      <title>التقرير المالي - ${normalizeArabicText(data.schoolName)}</title>
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <link rel="preconnect" href="https://fonts.googleapis.com">
      <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
      <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700&display=swap" rel="stylesheet">
    ...
    <body>
      <div class="report-container">
        <div class="report-header">
          ...
          <p class="report-school">${normalizeArabicText(data.schoolName)}</p>
        </div>
        ...
        <div class="report-info">
          ...
          <div class="report-info-item">
            <span>الصف: </span>
            <span class="report-info-value">${normalizeArabicText(data.grade)}</span>
          </div>
        </div>
        ...
        <div class="report-section">
          ...
          <div class="stats-container">
            ...
            <div class="stat-card">
              ...
              <div class="stat-value">${formatArabicCurrency(data.totalFees)}</div>
            </div>
            <div class="stat-card">
              <div class="stat-title">إجمالي المحصّل</div>
              <div class="stat-value">${formatArabicCurrency(data.totalCollected)}</div>
            </div>
            <div class="stat-card">
              <div class="stat-title">إجمالي المتبقي</div>
              <div class="stat-value">${formatArabicCurrency(data.totalPending)}</div>
            </div>
          </div>
          ...
          <div class="progress-container">
            <div class="progress-label">
              ...
              <span>${formatArabicCurrency(data.totalCollected)} / ${formatArabicCurrency(data.totalFees)}</span>
            </div>
          ...
          <table class="grade-payments">
            ...
            <tbody>
              ${data.paymentsByGrade.map(item => `
                <tr>
                  <td>${normalizeArabicText(item.grade)}</td>
                  <td>${item.count}</td>
